...
class SixgillActionResultProcessor(object):
    def __init__(self, siemplify, manager):
        ...
        self.table_columns =  ("Description, Created, Modified, Sixgill DVE Score Current," 
    ...
    def create_insights(self, event_obj):
        ...
        try:
            ...
            high_date = x_sixgill_info.get("score", {}).get("highest", {}).get("date", "")
            insight_record += f"Sixgill DVE Score Highest Ever Date: {high_date}\n"
            high_val = x_sixgill_info.get("score", {}).get("highest", {}).get("value", "")
    ...
    def create_table(self, stix_obj):
        ...
        try:
            ...
            table_rows.append(x_sixgill_info.get("score", {}).get("highest", {}).get("date", ""))
            highestEverValue = x_sixgill_info.get("score", {}).get("highest", {}).get("value", "")
            table_rows.append(str(highestEverValue))
            exploitabilityScore = x_sixgill_info.get("score", {}).get("previouslyExploited", "")
        ...
        for entity in self.siemplify.target_entities:
            try:
                if entity.entity_type == EntityTypes.CVE:
                    enrich_result = self.sixgill_manager.query_sixgill(entity.identifier)
                else:
        ...
        for entity in self.siemplify.target_entities:
            try:
                ...
                if enrich_result:
                    ...
                    self.siemplify.create_case_insight("Cybersixgill", entity.identifier, insight, entity.identifier, 0, 1)
                    self.siemplify.result.add_entity_table(entity.identifier, final_table)
