...
class LuminarManager:
    ...
    def test_connectivity(self):
        ...
        try:
            response = requests.request(
                "POST", req_url, headers=req_headers, data=payload, timeout=TIMEOUT
            )
    ...
    def get_access_token(self):
        ...
        try:
            req_url = (
                    self.base_url + "/externalApi/realm/" + self.account_id + "/token"
            )
            req_headers = {"Content-Type": "application/x-www-form-urlencoded"}
            ...
            response = requests.request("POST", req_url, headers=req_headers, data=payload, timeout=TIMEOUT)
            if response.ok:
                return response.json()["access_token"] if response.json()["access_token"] else False
