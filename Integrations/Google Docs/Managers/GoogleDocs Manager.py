...
class GoogleDocsManager:
    ...
    def test_connectivity(self):
        response = self._drive_service.files().list(q="mimeType='image/jpeg'", spaces='drive',fields='nextPageToken, files(id, name)', pageToken=None).execute()
    def __insert_permission__(self, file_id, role, user_emails_to_add):
        ...
        for email in emails:
            ...
            self._drive_service.permissions().create(fileId=file_id, body=user_permission, fields='id',sendNotificationEmail=False).execute()
    def create_document(self, title,role, emails):
        ...
        file_metadata_json = self._drive_service.files().get(fileId=file_id,fields='*').execute()
        return file_metadata_json
    def execute_request(self, document_id, requests_obj):
        result = self._doc_service.documents().batchUpdate(documentId=document_id, body={'requests': requests_obj}).execute()
        return result