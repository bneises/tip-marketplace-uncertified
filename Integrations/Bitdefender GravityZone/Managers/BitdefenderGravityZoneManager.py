...
# Doc: Docs are available via the Bitdefender GravityZone portal (PDF is available for download)

# ==============================================================================
# title          :BitdefenderGravityZoneManager.py
# description    :This Module contains all Bitdefender GravityZone Control Center API functionality
# author         :Daniel Harvey
# date           :27-DEC-2020
# python_version :3.7
# product_version: V1
# Doc            :https://github.com/snags141/SiemplifyIntegration_BitdefenderGravityZone
# ==============================================================================
...
ACTION_STATUSES = {"None" : 0, "Pending remove": 1, "Pending restore": 2, "Remove failed": 3, "Restore failed": 4, "Pending Save": 16, "Failed Save": 17}
...
class BitdefenderGravityZoneManager(object):
    def __init__(self, api_key: str, verify_ssl: bool = False):
        self.authorization = base64.b64encode((api_key + ":").encode("UTF-8")).decode("UTF-8")
        self.session = requests.Session()
        self.session.verify = verify_ssl
        self.session.headers.update({"Content-Type": "application/json", "Authorization": "Basic {0}".format(self.authorization)})
        ...
        if status_code is not 200:
            if status_code == -1:
                raise BitdefenderGravityZoneManagerError(
                    "{}: {} {}".format(status_code, "Unable to validate status code of request", response)
                )
            else:
                raise BitdefenderGravityZoneManagerError(
                        "{}: {} {}".format(response.status_code, HTTP_ERRORS.get(response.status_code), response)
                    )
    ...
    def get_endpoints_list(self, access_url, parent_id, endpoints, filter_best, filter_exchange, 
        ...
        if endpoints == "Managed":
            ...
            if endpoints == "Unmanaged":
                params['isManaged'] = False
        params['filters'] = {}
        params['filters']['security'] = {}
        params['filters']['security']['management'] = {"managedWithBest": filter_best, "managedExchangeServers": filter_exchange, "managedRelays": filter_relays, "securityServers": filter_security_servers}
        params['filters']['depth'] = {"allItemsRecursively": filter_depth_allrecursive}
        if filter_ssid:
            if not 'details' in params['filters']:
        ...
        if filter_name:
            ...
            if len(filter_name) > 0 and len(filter_name) < 3:
                raise BitdefenderGravityZoneManagerError(
                    "{}: {} {}".format("get_endpoints_list", "Minimum length required for 'name' filter is 3 characters.", params)
                )
            else:
    ...
    def blocklist_hashes_list(self, access_url):
        ...
        params = {}
        res = self.control_center_paged(INCIDENTS_URL.format(access_url), method, params)
    ...
    def blocklist_hashes_remove(self, access_url, hash_id):
        ...
        res = self.control_center_put(INCIDENTS_URL.format(access_url), method, params)[0]
    ...
    def blocklist_hashes_add(self, access_url, hash_csv, source_info):
        ...
        if hashes_unknown:
            raise BitdefenderGravityZoneManagerError(
                "{}: {} {}".format("blocklist_hashes_add","Unsupported hashes were found and not submitted. At this time, Bitdefender only accepts SHA256 or MD5.",hashes_unknown)
            )
        
        return { "result": { "hashes_sha256": hashes_sha256, "hashes_md5": hashes_md5, "hashes_failed": hashes_unknown }}
    
    def task_create_scan(self, access_url, target_ids, scan_type, task_name, scan_depth, scan_paths):
        """
        Create a scan task
        :return: {bool} true == success
        """
        ...
        if scan_type == "Aggressive":
            ...
            if scan_type == "Permissive":
                scan_depth_int = 3
        scan_path_list = scan_paths.split(",")
            
        params = {"targetIds": target_id_list,
            "type": scan_type_int,
            "name": task_name,
            "customScanSettings": {
    ...
    def task_create_scan_macaddr(self, access_url, target_macaddrs, scan_type, task_name, scan_depth, scan_paths):
        """
        Create a scan task
        :return: {bool} true == success
        """
        ...
        if len(target_macaddrs_list>100):
            raise BitdefenderGravityZoneManagerError(
                "{}: {} {}".format("task_create_scan_macaddr","Too many MAC addresses were given. Max 100.",target_macaddrs_list)
            )
        scan_type_int = 1
        if scan_type == "Full":
            scan_type_int = 2
        ...
        params = {"macAddresses": target_macaddrs_list,
            "type": scan_type_int,
            "name": task_name,
            "customScanSettings": {
                "scanDepth": scan_depth_int,
    ...
    def control_center_put(self, url, method, params):
        ...
        if 'error' in json_data:
            raise BitdefenderGravityZoneManagerError(
                "{}: {} {}".format(json_data['error']['message'],json_data['error']['code'],json_data['error']['data'])
            )
    ...
    def network_inventory_list(self, access_url, parent_id, filter_groups, filter_ec2, filter_computers, 
        ...
        params['filters'] = {}
        params['filters']['type'] = {"groups":filter_groups, "ec2Instances": filter_ec2, "computers": filter_computers, "virtualMachines": filter_vms}
        params['filters']['security'] = {}
        params['filters']['security']['management'] = {"managedWithBest": filter_best, "managedExchangeServers": filter_exchange, "managedRelays": filter_relays, "securityServers": filter_security_servers}
        params['filters']['depth'] = {"allItemsRecursively": filter_depth_allrecursive}
        ...
        if len(filter_name) > 0 and len(filter_name) < 3:
            raise BitdefenderGravityZoneManagerError(
                "{}: {} {}".format("network_inventory_list", "Minimum length required for 'name' filter is 3 characters.", params)
            )
        else:
            if len(filter_name) > 2:
                params['filters']['details']['name'] = filter_name
    ...
    def quarantine_item_list(self, access_url, endpoint_id, service, filter_threat_name, filter_start_date, filter_end_date, filter_file_path, filter_ip_addr, filter_action_status):
        """
        ...
        res = self.control_center_paged(QUARANTINE_URL.format(access_url) + "/" + service.lower(), method, params)
    ...
    def policy_details(self, access_url, policy_id):
        ...
        res = self.control_center_put(POLICIES_URL.format(access_url), method, params)[0]
    ...
    def integration_aws_ec2_usage(self, access_url, target_month):
        ...
        res = self.control_center_put(INTEGRATIONS_URL.format(access_url), method, params)[0]
    ...
    def quarantine_item_remove(self, access_url, quarantine_item_ids, service):
        ...
        res = self.control_center_put(QUARANTINE_URL.format(access_url) + "/" + service.lower(), method, params)[0]

        return { "result": res.get('result', False)}
        
    def quarantine_item_restore(self, access_url, quarantine_item_ids, service, add_exclusion, restore_location):
        """
    ...
    def quarantine_item_exchange_restore(self, access_url, quarantine_item_ids, username, password, email, ews_url):
        """
        ...
        res = self.control_center_put(QUARANTINE_URL.format(access_url) + "/exchange", method, params)[0]
    ...
    def quarantine_item_add(self, access_url, endpoint_ids, file_path):
        ...
        params = {"endpointIds": endpoint_ids_list,
            "filePath": file_path
        }
        
        res = self.control_center_put(QUARANTINE_URL.format(access_url), method, params)[0]
