...
from constants import IGNORED_INTEGRATIONS, AVAILABLE_CONTENT
...
def id_validator(resource, fields_to_compare, id_field, current_state):
    ...
    current = next((x for x in current_state if all(x[y] == resource[y] for y in fields_to_compare)), None)
    if current:
    ...
    try:
        ...
        if features["Dynamic Parameters"]:
            ...
            for dynParam in gitsync.content.get_dynamic_parameters():
                siemplify.LOGGER.info(f"Adding dynamic parameter {dynParam.get('name')}")
                gitsync.api.add_dynamic_env_param(id_validator(dynParam, "name", "id", current_parameters))
        ...
        if features["Environments"]:
            ...
            for environment in gitsync.content.get_environments():
                if environment.get("name") in all_environments_names:
                    ...
                    siemplify.LOGGER.info(f"Updating environment {environment.get('name')}")
                else:
                    siemplify.LOGGER.info(f"Adding environment {environment.get('name')}")
        ...
        if features["Integration Instances"]:
            ...
            for instance in gitsync.content.get_integration_instances():
                if instance['integrationIdentifier'] not in IGNORED_INTEGRATIONS:
                    current = next((x for x in current_instances if
                                    x["environmentIdentifier"] == instance["environment"] and
                                    x["integrationIdentifier"] == instance["integrationIdentifier"] and
                                    x["instanceName"] == instance["settings"]["instanceName"]), None)
                    if current:
                        siemplify.LOGGER.info(f"Updating {instance['settings']['instanceName']}")
                        instance_to_update = current
                    else:
                        siemplify.LOGGER.info(f"Installing {instance['settings']['instanceName']}")
                        instance_to_update = gitsync.api.create_integrations_instance(instance['integrationIdentifier'],
                                                                                      instance["environment"])
                    for i in instance["settings"]["settings"]:
                        i["integrationInstance"] = instance_to_update['identifier']

                    gitsync.api.save_integration_instance_settings(instance_to_update['identifier'],
                                                                   instance["settings"])

        if features["Playbooks"]:
            siemplify.LOGGER.info("========== Playbooks ==========")
            for playbook in gitsync.content.get_playbooks():
        ...
        if features["Case Stages"]:
            ...
            for stage in gitsync.content.get_stages():
                gitsync.api.add_case_stage(id_validator(stage, "name", "id", current_stages))
        ...
        if features["Case Close Reasons"]:
            ...
            for cause in gitsync.content.get_case_close_causes():
                gitsync.api.add_close_reason(id_validator(cause, ("rootCause", "forCloseReason"), "id", current_causes))

        if features["Case Title Settings"]:
            siemplify.LOGGER.info("Installing case title settings")
            gitsync.api.save_case_title_settings(gitsync.content.get_case_titles())
        ...
        if features["Mappings"]:
            ...
            for mapping in gitsync.content.get_mappings():
                gitsync.install_mappings(mapping, True)
        ...
        if features["Networks"]:
            ...
            for network in gitsync.content.get_networks():
                gitsync.api.update_network(id_validator(network, "name", "id", current_networks))

        if features["Domains"]:
            siemplify.LOGGER.info("Installing domains")
            current_domains = gitsync.api.get_domains()
            for domain in gitsync.content.get_domains():
                gitsync.api.update_domain(id_validator(domain, "domain", "id", current_domains))
        ...
        if features["Email Templates"]:
            ...
            for template in gitsync.content.get_email_templates():
                gitsync.api.add_email_template(id_validator(template, "name", "id", current_templates))
