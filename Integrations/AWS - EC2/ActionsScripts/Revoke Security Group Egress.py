...
def main():
    ...
    try:
        revoked_security_group = ec2_manager.revoke_security_group_egress(group_id, ip_permisssions_dict)
        siemplify.LOGGER.info(f"The outbound ip permissions of the security group id {group_id} were revoked successfully" )
        siemplify.result.add_result_json(revoked_security_group)
        print(json.dumps(revoked_security_group))
        
    except Exception as e:
        siemplify.LOGGER.error(f"Error occured when revoking the outbound ip permissions of the secrurity group id {group_id}.\nError: {e}")
