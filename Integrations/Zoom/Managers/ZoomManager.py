...
class ZoomManager:
    ...
    def __init__(
        ...
        if all((account_id, client_id, client_secret)):
            ...
            self.session.headers["Authorization"] = "Bearer {0}".format(jwt_token)
    ...
    def login(self, account_id: str, client_id: str, client_secret: str):
        ...
        try:
            self.logger.info(f"Authentication: requesting access token, params: {params}")
            response = self.session.post(
                OAUTH_TOKEN_URL, params=params, auth=(client_id, client_secret)
            )
    ...
    def list_meetings(self, user_email: str) -> Optional[dict]:
    ...
    def list_webinars(self, user_email: str) -> Optional[dict]:
    ...
    def list_recordings(self, user_email: str) -> Optional[dict]:
    ...
    def create_meeting(
        ...
        if meeting_type == Meeting.SCHEDULED:
            ...
            if timezone and timezone not in TIMEZONES:
                raise IOError(f"Timezone should be one of {TIMEZONES}")
        ...
        try:
            self.logger.info(f"Performing POST request. URL: {create_meeting_url}, payload: {payload}")
        ...
        if response.status_code != HTTPStatus.CREATED:
            if response.status_code is HTTPStatus.NOT_FOUND:
                raise NotFoundError(
                    f"Cannot create a meeting: {response.content}"
                )
    ...
    def create_user(
        self, first_name: str, last_name: str, email: str, user_type: str
    ) -> dict:
        ...
        if user_type not in USER_TYPES.keys():
            raise IOError(f"User type should be one of {USER_TYPES}")
        ...
        try:
            self.logger.info(f"Performing POST request. URL: {create_user_url}, payload: {payload}")
    ...
    def _get_request(self, url: str, path_params: dict = None) -> dict:
        ...
        try:
            self.logger.info(f"Performing GET request. URL: {url}, path_params: {path_params}")
