# ==============================================================================
# title           :PulsediveManager.py
# description     :This Module contain all pulsedive API functions.
...
from constants import PER_PAGE_ITEMS_COUNT, EMAIL_TYPE, DOMAIN_TYPE, DEFAULT_COMMENTS_COUNT, IN_PROGRESS, COMPLETED, \
DEFAULT_NEWS_COUNT
from exceptions import PulsediveNotFoundException, UnauthorizedException, PulsediveBadRequest, PulsediveException
...
REQUEST_PARAM = {
    "key": "",
    "pretty": "1"  # Pretty-print the results
}
...
class PulsediveManager(object):
    ...
    def get_indicator_data(self, indicator_id=None, indicator=None, get_links=False, get_properties=False,
                           schema=False, historical=False, retrieve_comments=False, comment_limit=DEFAULT_COMMENTS_COUNT):
        ...
        if indicator is not None:
            ...
            raise PulsediveException(
                u"Indicator ID or Indicator required and none were provided"
            )
        ...
        return self.parser.build_indicator_object(indicator_data.json(), entity_type='url', comments=comments)
    ...
    def scan_indicator(self, indicator=None, base64_data=None, scan_type="0"):
        ...
        if indicator is not None:
            ...
            raise PulsediveException(
                u"Indicator or Indicator base64 required and none were provided"
            )
        ...
        return response.json().get(u'qid')
    
    # Don't think this is implemented
    def parse_scan_data(self, scan_results, retrieve_comments=False, comment_limit=DEFAULT_COMMENTS_COUNT):
        comments = None
        if retrieve_comments:
    ...
    def get_scan_indicator_data(self, indicator, retrieve_comments=False, comment_limit=DEFAULT_COMMENTS_COUNT, completed_scan_id=""):
        indicator_scan_data = self.get_scan_data(completed_scan_id)
        
        if retrieve_comments:
            comments = self.get_comments(indicator_scan_data.json().get('data'), limit=comment_limit)
        
        return self.parser.build_indicator_object(indicator_scan_data.json().get('data'), entity_type='url', 
                                                  comments=comments)

    def submit_indicator(self, indicator=None, actions="submit", indicator_type="indicator", probe=0, indicator_risk="unknown",
                         indicator_threat=[], stamp_seen=None, indicator_activate=None,
                         indicator_comment="Submitted by Siemplify", indicator_attributes=None):
    ...
    def get_threats(self, threat_id=None, threat_name=None, get_linked_indicators=False, retrieve_comments=False,
                    comment_limit=DEFAULT_COMMENTS_COUNT, split_risk=False, news_limit=DEFAULT_NEWS_COUNT):
        request_url = self._get_full_url('info')
        params = copy.deepcopy(REQUEST_PARAM)
        comments = None
        ...
        if threat_name:
            ...
            raise PulsediveException(
                u"Threat ID or threat name required and none were provided"
            )
        ...
        return self.parser.build_threat_object(threat_data.json(), comments=comments, 
        news=news)
    ...
    # Not Implemented
    def get_linked_indicators(link_indicators_params):
        # Not Implemented
