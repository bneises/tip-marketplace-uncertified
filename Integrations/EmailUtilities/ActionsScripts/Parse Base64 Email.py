    ...
    if body_length <= slice_step:
        ...
        for ptr_end in range(slice_step, body_length, slice_step):
            if ' ' in body[ptr_end - 1:ptr_end]:
                while not (
                        EmailParserRegex.window_slice_regex.match(body[ptr_end - 1:ptr_end]) or ptr_end > body_length):
...
def parse_msg(msg, blacklist, is_whitelist, stop_transport):
    ...
    for _attachment in x_msg.attachments:
        ...
        if _attachment.type in 'msg':
            _attached_json = fill_json(_attachment.data, msox_obj['EmbeddedMessage']['properties'], blacklist,
                                       is_whitelist, stop_transport)  # , attached_obj.as_dict())
            try:
                _attached_json['body'].append(
                    body(base64.b64encode(_attachment.data.compressedRtf).decode(), "text/base64"))
            ...
            for _attach_attached in _attachment.data.attachments:
                _attached_json['attachment'].append(
                    attachment(filename=_attach_attached.shortFilename, content=_attach_attached.data))
            ...
            _cur_json['attachment'].append(
                attachment(filename=msox_obj['AttachLongFilename'], content=_attachment.data))
...
def process_attachment(attachment, blacklist, is_whitelist, stop_transport):
    ...
    try:
        attached_parsed = EmailParser.decode_email_b(attached_msg, include_raw_body=True, include_attachment_data=True,
...
def main():
    ...
    blacklist = list(
        set([x.strip().lower() for x in (siemplify.parameters.get("Blacklisted Headers") or "").split(",")]))
    ...
    try:
        if olefile.isOleFile(content):
            ...
            m = EmailParser.decode_email_b(content, include_raw_body=True, include_attachment_data=True,
                                           email_force_tld=True)
            ...
            try:
                ...
                if 'attachment' in nested_email:
                    ...
                    for nested_attachment in nested_email['attachment']:
                        ...
                        try:
                            nested_nested_email = process_attachment(nested_attachment, blacklist, is_whitelist,
                                                                     stop_transport)
                            ...
                            nested_emails[nested_attachment['hash']['md5']] = {
                                'filename': nested_attachment['filename'], 'email': nested_nested_email}
