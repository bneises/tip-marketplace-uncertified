...
def hash_headers(hasher, canonicalize_headers, headers, include_headers,
    ...
    cheaders = canonicalize_headers.canonicalize_headers(
        {b'dkim-signature': RE_BTAG.sub(b'\\1', sigheader[
            b'dkim-signature'])})  # the dkim sig is hashed with no trailing crlf, even if the
...
class DomainSigner(object):
    ...
    def __init__(self, message=None, logger=None, signature_algorithm=b'rsa-sha256',
                 minkey=1024, linesep='\r\n', info_content=False, timeout=5,
                 tlsrpt=False, headers=None):
        if headers:
            b_header = {}
    ...
    '''
        ...
        if self.info_content:
            self.logger.info("signed for %s: %r" % (sig_header[b'dkim-signature'], h.hashed()))
        signature = base64.b64decode(re.sub(br"\s+", b"", sig[b'b']))
        if self.ktag == b'rsa':
            try:
                res = RSASSA_PKCS1_v1_5_verify(h, signature, self.pk)
                self.logger.info("%s valid: %s" % (sig_header[b'dkim-signature'], res))
            ...
            try:
                ...
                self.logger.info("%s valid" % (sig_header[0]))
                return True
...
class ARC(DomainSigner):
            ...
            if x.lower() in ARC.ARC_HEADERS:
                ...
                if m is not None:
                    try:
                        ...
                        self.logger.info("invalid instance number %s: '%s: %s'" % (m.group(1), x, y))
                else:
    ...
    def sign(self, selector, domain, privkey, srv_id, include_headers=None,
        ...
        if instance == 1 and chain_validation_status != CV_None:
            ...
            self.logger.info("no previous AR arc results found and instance > 1, chain terminated")
            return []
    ...
    def verify(self, dnsfunc=get_txt):
        ...
        for result in result_data:
            if result['cv'] == CV_Fail:
                ...
                return CV_Fail, result_data, "ARC-Seal[%d] reported invalid status %s" % (
    ...
    def verify_instance(self, arc_headers_w_instance, instance, dnsfunc=get_txt):
        ...
        raw_ams_header = [(x, self.headers[x]) for x in self.headers if x.lower() == b'arc-message-signature'][0]
...
def sign(message, selector, domain, privkey, identity=None,
    ...
    return d.sign(selector, domain, privkey, identity=identity, canonicalize=canonicalize,
