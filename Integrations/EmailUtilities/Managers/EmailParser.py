...
class EmlParser:
    ...
    def parse_routing(self, entries) -> dict:
        ...
        for entry in entries:
            try:
                ...
                logger.debug('Invalid IP in received line - "{}"'.format(entry))
                try:
    ...
    def parse_email(self) -> dict:
        ...
        try:
            ...
            for received_line in self.msg.get_all('received', []):
                ...
                ips_in_received_line = EmailParserRegex.ipv6_regex.findall(received_line_flat) + \
        ...
        headers_struc['received_domains_internal'] = list(set(headers_struc['received_domains_internal']))
        ...
        if body_length <= slice_step:
            ...
            for ptr_end in range(slice_step, body_length, slice_step):
                if ' ' in body[ptr_end - 1:ptr_end]:
                    while not (EmailParserRegex.window_slice_regex.match(
                            body[ptr_end - 1:ptr_end]) or ptr_end > body_length):
    ...
    def headeremail2list(self, header: str) -> typing.List[str]:
        ...
        try:
            ...
            field = email.utils.getaddresses(EmailParserDecode.workaround_bug_27257(self.msg, header))
    ...
    def get_raw_body_text(self, msg: email.message.Message) -> typing.List[
    ...
    def prepare_multipart_part_attachment(self, msg: email.message.Message, counter: int = 0) -> typing.Dict[
        str, typing.Any]:
