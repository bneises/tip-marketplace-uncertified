...
def main(is_test_run):
    ...
    credentials_json = siemplify.extract_connector_param(
        param_name="Credentials Json", is_mandatory=True
    )
    sheet_id = siemplify.extract_connector_param(
        param_name="Sheet Id", is_mandatory=True
    )
    ...
    alert_name_column_index = siemplify.extract_connector_param(
        param_name="Alert Name Column Index", is_mandatory=True
    )
    ...
    print("{0} alert was successfully generated.".format(len(alerts)))
