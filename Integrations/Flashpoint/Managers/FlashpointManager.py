# ==============================================================================
# title           :FlashpointManager.py
# description     :This Module contain all Flashpoint Management functionality
# date            :5.15.20
# python_version  :3.7
# ==============================================================================
# =====================================
...
#              CLASSES                #
# =====================================
class FlashpointManager(object):
    def __init__(self, api_key, proxy_server_address=None, proxy_username=None, proxy_password=None):
    ...
    def indicators_custom_query(self, results_limit, start_date, end_date, search_tags, query, sort_timestamp,
                                attributes_types, basetypes_path=None):
        if sort_timestamp.lower() == "descending":
            sort_timestamp = "desc"
        elif sort_timestamp.lower() == "ascending":
            sort_timestamp = "asc"
        params = {"limit": int(results_limit), "start_date": start_date, "end_date": end_date,
                  "search_tags": search_tags, "query": query, "sort_timestamp": sort_timestamp,
    ...
    def IOC_Enrichment(self, query, results_limit, SORT_RESULTS_TIMESTAMP):
        data = {"query": query, "limit": results_limit, "sort_timestamp": SORT_RESULTS_TIMESTAMP}
    ...
    def search_CCM(self, last_timestamp, time_now, limit=100):
        params = {
            "q": "basetypes:(credential-sighting) +created_at.timestamp:[{0} TO {1}]".format(last_timestamp, time_now),
            "limit": limit}
        response = self.session.get(f"{API_ROOT}/all/search", params=params)
        response.raise_for_status()
        try:
