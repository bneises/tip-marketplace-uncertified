import requests
import time, copy, requests, json, os
...
class DataDogManager(object):
    ...
    def get_processes_by_specific_tags(self, name_space, deployment=None):
        ...
        if deployment is not None:
            params = {'tags': f'kube_namespace:{name_space},kube_deployment:{deployment}'}
        else:
    ...
    def get_timeseries_point_metrics_cpu_pod(self, pod_name, from_unixtime, to_unixtime):

        url = 'https://api.datadoghq.com/api/v1/query'
        query = {'query':'avg:kubernetes.cpu.system.total{{pod_name:{0}}}by{{pod_name,kube_namespace}}'.format(pod_name),
                 'from': from_unixtime, 'to': to_unixtime}
        response = self.session.get(url, params=query)
        response.raise_for_status()
        try:
            response.json()
    ...
    def get_timeseries_point_metrics_memory_pod(self, pod_name, from_unixtime, to_unixtime):

        url = 'https://api.datadoghq.com/api/v1/query'
        query = {'query': 'avg:kubernetes.memory.usage_pct{{pod_name:{0}}}by{{pod_name,kube_namespace}}'.format(pod_name),
                 'from': from_unixtime, 'to': to_unixtime}
        response = self.session.get(url, params=query)
        response.raise_for_status()
        try:
    ...
    def get_aws_rds_metrics_memory(self, db_instance_identifier, from_unixtime, to_unixtime):

        url = 'https://api.datadoghq.com/api/v1/query'
        query = {'query': 'avg:aws.rds.freeable_memory{{dbinstanceidentifier:{0}}}by{{dbinstanceidentifier}}'.format(
            db_instance_identifier), 'from': from_unixtime, 'to': to_unixtime}
        response = self.session.get(url, params=query)
        response.raise_for_status()
        try:
            response.json()
    ...
    def get_aws_rds_metrics_cpu(self, db_instance_identifier, from_unixtime, to_unixtime):

        url = 'https://api.datadoghq.com/api/v1/query'
        query = {
            'query':'avg:aws.rds.cpuutilization{{dbinstanceidentifier:{0}}}by{{dbinstanceidentifier}}'.format(
                db_instance_identifier), 'from': from_unixtime, 'to': to_unixtime}
        response = self.session.get(url, params=query)
        response.raise_for_status()
        try:
    ...
    def get_aws_rds_metrics_storage(self, db_instance_identifier, from_unixtime, to_unixtime):

        url = 'https://api.datadoghq.com/api/v1/query'
        query = {'query': 'avg:aws.rds.free_storage_space{{dbinstanceidentifier:{0}}}by{{dbinstanceidentifier}}'.format(
            db_instance_identifier), 'from': from_unixtime, 'to': to_unixtime}
        response = self.session.get(url, params=query)
        response.raise_for_status()
        try:
    ...
    def get_all_hosts(self, filters, sort_direction, include_hosts_metadata= True):
        ...
        query = {'filter': filters, 'sort_dir': sort_direction,
    ...
    def get_datadog_events(self, start_time, end_time, sources, tags='monitor', priority='all', unaggregated=True):

        url = f'{BASE_URL}/api/v1/events'
        query = {}
        data = {'start': start_time, 'end': end_time, 'sources': sources,'tags':tags, 'priority':priority, 'unaggregated': unaggregated}
