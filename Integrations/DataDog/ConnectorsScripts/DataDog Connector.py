...
def create_alert(siemplify, created_event, base_url):
    ...
    alert_info.rule_generator = created_event.get(
        'metric')  # The name of the siem rule which causes the creation of the alert.
    alert_info.start_time = created_event.get(
        'date_happened')  # Time should be saved in UnixTime. You may use SiemplifyUtils.convert_datetime_to_unix_time, or SiemplifyUtils.convert_string_to_datetime
    ...
    alert_info.device_product = created_event.get(
        'device_product')  # The field will be fetched from the Original Alert. If you build this alert manually, state the source product of the data. (ie: ActiveDirectory, AntiVirus)
...
def create_event(siemplify, event_full_details, base_url):
    ...
    siemplify.LOGGER.info(f"-------Started processing the Event {event_payload.get('monitor').get('name')}-------")
    ...
    event[
        "device_product"] = product_name  # ie: "device_product" is the field name that describes the product the event originated from.
...
def fetch_last_saved_timestamp(siemplify, time_now, max_days_back):
    ...
    if last_saved_timestamp > 0 and not is_test_run:
        ...
        siemplify.LOGGER.info(f"This is a real run. The last timestamp that was saved is:{last_saved_timestamp}")
...
def main(is_test_run):
    ...
    events_data = datadog_manager.get_datadog_events(last_saved_timestamp, time_now, sources, tags, priority,
                                                     unaggregated)
                ...
                if parent_alert_type == 'error' or parent_alert_type == 'warning':
                    ...
                    if event_parent_data.get("children") is not None:
                        ...
                        siemplify.LOGGER.info(
                            "The event {0} has related children events:\n{1}".format(event_parent_data.get('id'),
    ...
    if not is_test_run:
        ...
        siemplify.LOGGER.info(f"The latest timestamp to save is {max(collected_timestamps)}")
        new_timestamp_to_save = max(collected_timestamps)
        siemplify.save_timestamp(new_timestamp_to_save)
        siemplify.LOGGER.info(f"The new timestamp that was saved: {new_timestamp_to_save}.")
