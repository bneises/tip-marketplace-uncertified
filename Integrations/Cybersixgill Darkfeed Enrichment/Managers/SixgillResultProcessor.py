...
class SixgillActionResultProcessor(object):
    def __init__(self, siemplify, manager):
        ...
        self.filtered_keys = ["description", "sixgill_feedname", "sixgill_source", "sixgill_posttitle",
                               "sixgill_actor", "sixgill_postid", "labels", "sixgill_confidence", "sixgill_severity",
                               "created", "modified", "valid_from","external_reference"]
        self.ext_obj_keys = ["positive_rate","url","description","mitre_attack_tactic","mitre_attack_tactic_id","mitre_attack_tactic_url",
                                "mitre_attack_technique","mitre_attack_technique_id","mitre_attack_technique_url"]
        self.table_columns =  ("Description,Sixgill Feed Name,Sixgill Source,Sixgill Post Title,Sixgill Actor,Sixgill Post Id,Labels," 
    ...
    def format_output_message(self, output_message):
        try:
            ...
            if not len(self.success_entities) + len(self.faild_entities) + len(self.no_data_entities):
        ...
        try:
            for item in rec.get("external_reference", []):
                if item.get("source_name") == "VirusTotal":
                    ...
                    self.siemplify.LOGGER.warning(f"Unsupported External links - {item.get('source_name','')}")
        except Exception as err:
        ...
        try:
            ...
            for key in self.ext_obj_keys:
                if key in ["positive_rate","url"]:
                    val = vt_obj.get(key,"")
                    if val:
                        insight_record+= f"VirusTotal {key.replace('_',' ').title()}  :  {val}\n"
                        if key == "url":
                            links.append(val)
                else:
                    ...
                    if val:
                        if not key.startswith("mitre"):
                            insight_record+= f"Mitre {key.replace('_',' ').title()}  :  {val}\n"
                        else:
                            insight_record+= f"{key.replace('_',' ').title()}  :  {val}\n"
                            if key in ["mitre_attack_tactic_url","mitre_attack_technique_url"]:
                                links.append(val)
                rows.append(val)
            status = True
        except Exception as err:
        ...
        try:
            ...
            for key in self.filtered_keys:
                if key == "external_reference":
                    ext_status,insight_record,links,rows = self.get_external_obj_details(entity,rec,insight_record,links,rows)
                elif key == "labels":
                    ...
                    val = "https://portal.cybersixgill.com/#/search?q=_id:" + rec.get(key, "")
                    links.append(val)
                    insight_record+= f"{key.replace('_',' ').title()}  :  {val}\n"
                    rows.append(val)
                else:
    ...
    def get_action_output(self,entity,records):
        ...
        try:
            ...
            for rec in records:
                insight_status,insight_record,links,row = self.create_table_insights(entity,rec)
                if insight_status:
                ...
                if entity.entity_type.upper() in entity_types:
                    try:
                        ...
                        if entity.entity_type == EntityTypes.HOSTNAME:
                            enrich_status, enrich_result = self.sixgill_manager.query_sixgill("domain", entity.identifier)
                        elif entity.entity_type == EntityTypes.FILEHASH:
                            enrich_status, enrich_result = self.sixgill_manager.query_sixgill("hash", entity.identifier)
                        elif entity.entity_type == EntityTypes.ADDRESS:
                            enrich_status, enrich_result = self.sixgill_manager.query_sixgill("ip", entity.identifier)
                        elif entity.entity_type.upper() == EntityTypes.URL.upper():
                            enrich_status, enrich_result = self.sixgill_manager.query_sixgill("url", entity.identifier)
                        elif entity.entity_type.upper() == EntityTypes.THREATACTOR.upper():
                            enrich_status, enrich_result = self.sixgill_manager.query_sixgill("actor", entity.identifier)
                        else:
                            self.siemplify.LOGGER.error(f"Invalid Entity Type - {entity.entity_type} ")
                        if enrich_status and enrich_result:
                            ...
                            for rec in enrich_result:
                                for indicator_type, sub_type, value in STIX_REGEX_PARSER.findall(rec.get("pattern", "")):
                                ...
                                data_dict["Post ID"] = "https://portal.cybersixgill.com/#/search?q=_id:" + rec.get("sixgill_postid", "")
                                ...
                                for obj in external_reference:
                                    if obj.get("source_name", "") == "VirusTotal":
                                        data_dict["Virustotal PR"] = obj.get("positive_rate")
                                        data_dict["Virustotal Url"] = obj.get("url")
                                    if obj.get("source_name", "") == "mitre-attack":
                                        data_dict["Mitre Description"] = obj.get("description")
                                        data_dict["Mitre Tactic"] = obj.get("mitre_attack_tactic")
                                        data_dict["Mitre Tactic Id"] = obj.get("mitre_attack_tactic_id")
                                        data_dict["Mitre Tactic Url"] = obj.get("mitre_attack_tactic_url")
                ...
                if entity.entity_type.upper() in entity_types:
                    try:
                        ...
                        if enrich_status and enrich_result:
                            ...
                            output_status,table,insight = self.get_action_output(entity,enrich_result)
                            if output_status:
                                self.siemplify.result.add_entity_table(entity.identifier, table)
                                self.siemplify.create_case_insight("Sixgill", entity.identifier, insight, entity.identifier, 0, 1)
