...
class IPQSManager(object):
    ...
    def __init__(self, siemplify, api_key, data_json):
        ...
        self.url_table_columns = ("Unsafe, Domain, IP Address, Server, Domain Rank, DNS Valid, Parking, Spamming,"
        ...
        self.email_table_columns = ("Valid, Disposable, SMTP Score, Overall Score, First Name, Generic, Common,"
        ...
        self.phone_table_columns = ("Formatted, Local Format, Valid, Fraud Score, Recent Abuse, VOIP, Prepaid, Risky, "
    ...
    def format_output_message(self, output_message):
        try:
            ...
            if self.ipqs_con_issues:
                output_message += f"Enrichment failed for the following Entities - {','.join(self.failed_entities)}, "
            if not len(self.success_entities) + len(self.failed_entities) + len(self.no_data_entities):
    ...
    def get_action_output(self, response, enrich_type=None):
        ...
        elif enrich_type == IP_ENDPOINT:
            return self.ip_action_output(response)
        elif enrich_type == EMAIL_ENDPOINT:
            return self.email_action_output(response)
        elif enrich_type == PHONE_ENDPOINT:
    ...
    def email_action_output(self, response):
        ...
        try:
                ...
                if data_item in response:
                    if data_item in self.timestamp_data_items:
                        for time_stamp_item in self.timestamp_items:
                            key = data_item.replace('_', ' ').title() + " " + time_stamp_item.replace('_', ' ').title()
            ...
            row_data = ",".join(rows)
            table.append(row_data)
            insight_record += self.email_address_risk_scoring(fraud_score, disposable, valid)
        except Exception as err:
                ...
                if entity.entity_type in entity_types:
                    try:
                        ...
                        if enrich_status and enrich_result:
                            ...
                            table, insight, data_dict = self.get_action_output(enrich_result, enrich_type)
                            self.siemplify.result.add_entity_table(entity.identifier, table)
                            self.siemplify.create_case_insight("IPQualityScore", entity.identifier, insight,
                                                               entity.identifier, 0, 1)
                            self.siemplify.result.add_json(entity.identifier, enrich_result)

                            # Enrichment Code
                            entity_dict = add_prefix_to_dict(data_dict, "IPQualityScore")
                            ...
                            output_message = f"Action execution failed with Error - {ipqs_message}"
                            self.siemplify.LOGGER.error(f"Enrich Error - {ipqs_message}")
                    except Exception as err:
