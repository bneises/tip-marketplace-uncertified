...
class GoogleDriveManager:
    ...
    def test_connectivity(self):
        response = self._service.files().list(q="mimeType='image/jpeg'",spaces='drive',fields='nextPageToken, files(id, name)',pageToken=None).execute()
    ...
    def list_permissions(self,file_id):
        ...
        permissions = self._service.permissions().list(fileId=file_id,fields='*').execute()
    ...
    def _get_binary_stream_from_file(self,file_id,file_metadata_json):
        ...
        if file_metadata_json['mimeType'] == 'application/vnd.google-apps.document':
            request = self._service.files().export_media(fileId=file_id, mimeType='application/vnd.openxmlformats-officedocument.wordprocessingml.document')

        elif file_metadata_json['mimeType'] == 'application/vnd.google-apps.presentation':
            request = self._service.files().export_media(fileId=file_id,mimeType='application/vnd.openxmlformats-officedocument.presentationml.presentation')

        elif file_metadata_json['mimeType'] == 'application/vnd.google-apps.spreadsheet':
            request = self._service.files().export_media(fileId=file_id,mimeType='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
    ...
    def download_file_to_path(self,file_id, folder_to_save):
        file_metadata_json = self._service.files().get(fileId=file_id,fields='*').execute()
        ...
        if return_value['type'] == 'application/vnd.google-apps.document':
            ...
            path_to_save = folder_to_save + "/" + return_value['name'] + '.' + return_value['type']
    ...
    def insert_permission(self, file_id,role, emails, sendNotificationEmail = True):
        ...
        res = self._service.permissions().create(fileId=file_id,body=user_permission,fields='id',sendNotificationEmail = sendNotificationEmail).execute()
        return res
    ...
    def upload_file_from_path(self,file_path):
        ...
        data = self._service.files().create(body=file_metadata,media_body=media,fields='*').execute()
    ...
    def copy_file(self, origin_file_id, copy_title):
        ...
        res =  self._service.files().copy(fileId=origin_file_id,fields="*", body=copied_file).execute()
        return res;
    ...
    def remove_permission(self,file_id,permission_id):
        ...
        """
        self._service.permissions().delete(fileId=file_id, permissionId=permission_id).execute()
    ...
    def get_file_metadata(self,file_id):
        ...
        """
        resource_metadata_json = self._service.files().get(fileId=file_id,fields='*').execute()
